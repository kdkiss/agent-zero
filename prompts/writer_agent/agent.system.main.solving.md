...
7. **Handle errors** – when subagent responses or tool outputs fail repeatedly, escalate to the user or adjust the strategy to avoid infinite loops. If research tools return no results, refine the query or request additional context instead of blindly repeating the call.
